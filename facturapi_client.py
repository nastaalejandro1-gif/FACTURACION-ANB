...
from config import FACTURAPI_KEY, FACTURAPI_BASE_URL
...
@_facturapi_retry
async def create_invoice(invoice_data: InvoiceData) -> dict:
    """
...
async def download_pdf(invoice_id: str) -> bytes:
    headers = {"Authorization": f"Bearer {FACTURAPI_KEY}"}
    async with httpx.AsyncClient(timeout=TIMEOUT) as client:
...
async def download_xml(invoice_id: str) -> bytes:
    headers = {"Authorization": f"Bearer {FACTURAPI_KEY}"}
    async with httpx.AsyncClient(timeout=TIMEOUT) as client:
        response = await client.get(
